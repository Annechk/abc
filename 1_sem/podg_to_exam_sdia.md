...
- Способ организации и представления данных в памяти компьютера
- Определение набора операций, которые можно выполнять над данными
- Учет расхода памяти и времени выполнения операций
- Удобство использования при решении конкретных задач

**Классификация структур данных:**
1. **Линейные структуры:** массивы, стеки, очереди, деки, списки
2. **Нелинейные структуры:** деревья, графы, тьюп
3. **Специализированные структуры:** хеш-таблицы, кучи
...
**Массивы:**
- Определение: упорядоченная коллекция элементов одного типа, расположенных в памяти последовательно
- Способы организации:
  * Статические массивы: размер определяется при компиляции, память в стеке
  * Динамические массивы: размер определяется во время выполнения, память в куче

**Стеки (LIFO — Last In, First Out):**
- Определение: структура данных, где элементы добавляются и удаляются с одного конца (вершины)
- Основные операции: push (добавление), pop (удаление), peek (просмотр вершины)
- Пример применения: обратный порядок строк, undo операции

**Очереди (FIFO — First In, First Out):**
- Определение: структура данных, где элементы добавляются в конец (rear) и удаляются с начала (front)
- Основные операции: enqueue (добавление), dequeue (удаление)
- Сложность операций: O(1)
- Применение: обработка задач по порядку, буферизация данных

**Деки (Double-Ended Queues):**
- Определение: двусторонняя очередь, позволяющая добавлять и удалять элементы с обоих концов
- Основные операции: pushFront, pushBack, popFront, popBack, isEmpty
- Частные случаи: стек (только одна операция) и очередь (ограниченные операции)

**Простые списки:**
- Определение: цепочка узлов, где каждый узел содержит данные и указатель на следующий узел
- Организация: использование указателей вместо индексов
- Основные операции: insert, delete, search, traverse

**Мультисписки:**
- Определение: список, где каждый узел содержит несколько указателей на другие узлы
- Применение: представление графов, сложных иерархических структур
...
**Преимущества:**
- O(1) временная сложность для всех операций (insert, search, delete)
...
**Принцип работы:**
1. Выбрать хеш-функцию случайно из универсального семейства
2. Использовать эту функцию для всех операций
3. Новый запуск программы может использовать другую функцию из семейства
4. Гарантируется распределение близко к идеальному
...
**Определение:**
Нелинейные структуры данных — это структуры, где элементы не расположены в линейной последовательности, а имеют иерархические или сетевые отношения между собой.

**Виды нелинейных структур:**

1. **Деревья:**
   - Определение: иерархическая структура с корневым узлом и подчинённым узлам
   - Примеры: бинарные деревья, AVL-деревья, красно-чёрные деревья
   - Применение: поиск, сортировка, базы данных

2. **Графы:**
   - Определение: набор вершин и рёбер, без иерархии
   - Примеры: ориентированные графы, взвешенные графы, неориентированные графы
   - Применение: сетевые задачи, навигация, социальные сети
